...
function PlantCard({image, name, price, id, plants, setPlants}) {
  ...
  function handleDelete(){
    ...
    .then(res => res.json())
    .then(() => {
      let avail = plants.filter(plantsy => plantsy.id !== id)
      setPlants(avail)
      alert(`Just deleted ${name}`)
    })
  ...
  return (
    <ul>
      <li className="card" data-testid="plant-item">
        ...
        <p>Price: {price}</p>
        <p>{id}</p>
        <button className={stock ? "primary" : ""} onClick={handleToggleStock}>
